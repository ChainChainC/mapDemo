...
func PlayerUpdatePos(c *gin.Context) {
	...
	if p, ok := model.PlayerIdMap[pupReq.Uuid]; ok {
		...
		if err := verifyJwt(p, &pupReq.Jwt); err != nil {
			c.JSON(200, gin.H{"code": 200, "data": "PlayerUpdatePos", "msg": err})
		}
		// 更新玩家位置
...
func PlayerJoinRoom(c *gin.Context) {
	...
	if p, ok := model.PlayerIdMap[pjrReq.Uuid]; ok {
		...
		if err := verifyJwt(p, &pjrReq.Jwt); err != nil {
			c.JSON(100, gin.H{"code": 100, "data": "PlayerJoinRoom", "msg": err})
		}
		if err := addPlayerIntoRoom(p, &pjrReq.RoomUuid); err != nil {
			c.JSON(100, gin.H{"code": 100, "data": "PlayerJoinRoom", "msg": err})
		}
		c.JSON(100, gin.H{"code": 100, "data": "PlayerJoinRoom", "msg": "玩家加入加入房间成功" + p.RoomId})
	} else {
		// TODO: 处理玩家不在线
		c.JSON(100, gin.H{"code": 100, "data": "PlayerJoinRoom", "msg": "玩家加入房间，玩家状态不在线"})
	}
}
...
func PlayerQuitRoom(c *gin.Context) {
	...
	if p, ok := model.PlayerIdMap[pqrReq.Uuid]; ok {
		...
		if err := verifyJwt(p, &pqrReq.Jwt); err != nil {
			c.JSON(100, gin.H{"code": 100, "data": "PlayerQuitRoom", "msg": err})
		}
		...
		// TODO:玩家未在服务器内登录记录
		c.JSON(100, gin.H{"code": 100, "data": "PlayerQuitRoom", "msg": "玩家不在线"})
	}
}
